/*
  # Fix can_user_approve_at_node Schema Qualification
  
  1. Problem
    - Function has search_path set to 'public' but doesn't use explicit schema qualification
    - This causes "relation does not exist" errors when the function executes
    - Results in approvals not showing up even when user has correct roles
  
  2. Solution
    - Add 'public.' prefix to all table references
    - Ensures function can find tables regardless of caller's search_path
  
  3. Changes
    - Update can_user_approve_at_node to use public.project_approvals, public.approval_workflow_canvas, public.user_role_assignments
*/
