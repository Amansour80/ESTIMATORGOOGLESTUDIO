/*
  # Rebuild start_approval_workflow with Correct Types

  ## Problem
  The function accepts text for project_type but the table expects project_type_enum.
  This causes "column is of type enum but expression is of type text" errors.

  ## Solution
  - Accept text parameter from client
  - Cast to enum when inserting into table
  - Properly handle all type conversions
*/
...
BEGIN
  ...
  INSERT INTO public.project_approvals (
    ...
    p_project_id,
    p_project_type::public.project_type_enum,  -- EXPLICIT CAST
    v_workflow_id,
    v_first_step_node_id,
    'pending'::public.approval_status,  -- EXPLICIT CAST
    p_submitted_by,
    v_project_data
  )
